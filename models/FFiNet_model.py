...
class FFiNetModel(nn.Module):
    def __init__(self, feature_per_layer: List, num_heads: int,
                 activation=nn.ELU(), residual: bool = True, 
...
class FFiLayer(nn.Module):
    def __init__(self, num_node_features: int, output_dim: int, num_heads: int,
        ...
        self.linear_angle = nn.Linear(2, output_dim * num_heads)
        self.linear_dihedral = nn.Linear(6, output_dim * num_heads)
    ...
    def init_params(self):
        ...
        nn.init.xavier_uniform_(self.linear_angle.weight)
        nn.init.xavier_uniform_(self.linear_dihedral.weight)
        nn.init.constant_(self.linear_angle.bias, 0) 
        nn.init.constant_(self.linear_angle.bias, 0) 
    ...
    def forward(self, data: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]):
        ...
        distance_matrix = self.linear_pos_bonded(distance_matrix_bonded).view(-1, self.num_heads, self.output_dim)
        if edge_attr is not None:
            distance_matrix_unbonded = torch.cat([distance_per_edge ** (-6),
                                           distance_per_edge ** (-12),
                                           distance_per_edge ** (-1)], dim=1)

            distance_matrix = torch.cat([self.linear_pos_bonded(distance_matrix_bonded[~edge_attr]).view(-1, self.num_heads, self.output_dim), 
                                self.linear_pos_unbonded(distance_matrix_unbonded[edge_attr]).view(-1, self.num_heads, self.output_dim)], dim=0)
